...
public class MaterialRepository implements Map<Integer, Material> {
    ...
    private static final String INSERT_MATERIAL = "insert into material_projecto (quantidade, motivo, projecto_id, stock_id) values (?,?,?,?)";
    private static final String UPDATE_MATERIAL = "update material_projecto set quantidade = ?, motivo = ?, projecto_id = ?, stock_id = ? where id_material = ?";
    ...
    private static final String SELECT_MATERIAL = "SELECT id_material, stock.descricao, material_projecto.quantidade, motivo, projecto_id, stock_id FROM material_projecto INNER JOIN stock ON material_projecto.stock_id = stock.id_stock WHERE id_material = ?";
    private static final String SELECT_MATERIAIS = "SELECT id_material, stock.descricao, material_projecto.quantidade, motivo, projecto_id, stock_id FROM material_projecto INNER JOIN stock ON material_projecto.stock_id = stock.id_stock";
    private static final String SELECT_BY_PROJECTO = "SELECT id_material, stock.descricao, material_projecto.quantidade, motivo, projecto_id, stock_id FROM material_projecto INNER JOIN stock ON material_projecto.stock_id = stock.id_stock where projecto_id = ?";
        ...
        try {
            ...
            try (ResultSet result = statement.executeQuery()) {
                if (result.next()) {
                    ...
                    material.setStock_id(result.getInt("stock_id"));
                }
            } finally {
                statement.close();
                connection.close();
    ...
    public Material put(Integer key, Material value) {
        ...
        try {
            ...
            statement.setString(1, (value.getQuantidade()));
            statement.setString(2, (value.getMotivo()));
            statement.setInt(3, (value.getProjecto_id()));
            statement.setInt(4, (value.getStock_id()));
